...
var BASE = { pixelSize: 4, shape: 0, rippleThickness: 0.10 };
  ...
  function buildSolid(gl, U, canvas, getDpr) {
    ...
    function sync() {
      ...
      var dark = isDark();
      /* The field peaks near 0.37 coverage, so the solid stays above it at
         both ends. Its lit faces are the sparse end of the solid, never sparser
         than the field behind it, which is what stopped it reading as a hole
         punched through the dots. */
      gl.uniform1f(U.uShapeLo, dark ? 0.58 : 1.05);
      gl.uniform1f(U.uShapeHi, dark ? 1.05 : 0.58);
      ok = true;
