...
var BASE = { pixelSize: 4, shape: 0, rippleThickness: 0.10 };
  ...
  function buildSolid(gl, U, canvas, getDpr) {
    ...
    function sync() {
      ...
      var dark = isDark();
      /* The field itself sits around 0.2 coverage, so the solid has to clear
         that at BOTH ends or its lit faces read as holes punched in the field
         rather than as the lit side of an object. */
      gl.uniform1f(U.uShapeLo, dark ? 0.42 : 1.25);
      gl.uniform1f(U.uShapeHi, dark ? 1.25 : 0.42);
      ok = true;
